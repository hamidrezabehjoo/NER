"""
Task: given a text, find all pieces of time expressions. For example, given the text:

This assignment is due Dec 5, 2022.

a system should find time expressions at the following character offsets:

(23, 26, Month-Of-Year)
(27, 28, Day-Of-Month)
(30, 34, Year)

which says that the characters "Dec" are a month, "5" is a day, and "2022" is a year.

The output format is Anafora XML, the same XML format as the training data.

python3 nn_2.py train model train
python3 nn_2.py predict model submission dev

python3 -m anafora.evaluate -r dev -p submission -e "*:<span>"
"""
import argparse
import os
...
def train(model_dir: Path,
    ...
    #print(n_words)
    #sys.exit()
    # put all documents on sentence level
    out = []
    for c in outputs:
        for t in c:
            out.append(t)
    ...
    inputs  = tf.keras.preprocessing.sequence.pad_sequences( inputs,  padding="post", maxlen=maxlen, truncating='post')
    outputs = tf.keras.preprocessing.sequence.pad_sequences( outputs, padding="post", maxlen=maxlen, truncating='post')

    # create a model  
    
    embedding_dim = 128
    num_words = len(nlp.vocab.vectors) + 1
    num_tags = len(labels)
	...
	tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(units=256, activation='tanh',\
...
def predict(model_dir: Path,
    ...
    for text_path, text, xml_path in iter_files(text_dir, "system"):
        ...
        token_indexes  = tf.keras.preprocessing.sequence.pad_sequences( token_indexes,  padding="post",\
                                                                        maxlen=256, truncating='post')
        #print(token_indexes.shape)
        #sys.exit()
        predictions = model.predict(token_indexes)
        token_labels = np.argmax(predictions, axis=-1).reshape(token_indexes.shape)
        #print(token_labels)
        #print('Hi', token_labels.shape)
        #sys.exit()
        # for debugging purposes, print out the first document's labels
        if debug:
            print_token_info(text, token_offsets, token_indexes, token_labels)
